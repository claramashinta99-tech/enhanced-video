...
async function runOptimized(input, output){
  const vf = "scale='min(1080,iw)':'min(1920,ih)':force_original_aspect_ratio=decrease,fps=60";
  const cmd = [
    '-i', input,
    '-c:v', 'libx264',
    '-preset', 'fast',
...
function outputName(){const base=file.name.replace(/\.[^.]+$/,'');return `${base}-optimized.mp4`}
$('#process').addEventListener('click',async()=>{if(!file||busy)return;resetResult();const c=t();busy=true;$('#process').disabled=true;$('#progress-wrap').classList.add('show');setProgress(3,c.loading);let input=null;const output='output.mp4';try{await ensureFFmpeg();setProgress(8,"Merekayasa ulang Bitrate & FPS (Proses ini mungkin memakan waktu)...");const ext=file.name.split('.').pop().toLowerCase();input=`input.${ext}`;await ffmpeg.writeFile(input,new Uint8Array(await file.arrayBuffer()));await runOptimized(input,output);const data=await ffmpeg.readFile(output);const bytes=data instanceof Uint8Array?data:new Uint8Array(data);const blob=new Blob([bytes],{type:'video/mp4'});setProgress(100,c.ready);resultURL=URL.createObjectURL(blob);const dl=$('#download');dl.href=resultURL;dl.download=outputName();dl.textContent=c.download;$('#result-title').textContent=c.ready;$('#result-meta').textContent=`Optimized · ${formatBytes(blob.size)}`;$('#result').classList.add('show');$('#show-result').disabled=false}catch(err){console.error(err);toast(c.failed);$('#progress-text').textContent=c.failed}finally{if(input)await deleteLocal(input);await deleteLocal(output);busy=false;$('#process').disabled=!file}});
