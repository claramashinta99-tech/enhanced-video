(()=>{
  ...
  async function inspect(){
    ...
    try{
      ...
      if(isCarouselCandidate(url,kind)){
        ...
        else if(kind.platform==='tiktok')throw new Error(res.data.detail||text('TikTok slideshow nggak bisa dibaca.','Could not read TikTok slideshow.'));
      }
      if(!data){
        const infoPath=kind.family==='social'?'/api/social/info':'/api/info';
        const res=await fetchJson(infoPath,url,inspectController.signal);
        if(run!==inspectRun)return;
        if(!res.r.ok){
      ...
      current={url,kind,data,mode:'video'};setThumb(data.thumbnail,data.title);titleEl.textContent=data.title||'Untitled';
  ...
  async function pollCarousel(jobId,run,button){
    while(run===downloadRun){
      await sleep(650);const r=await fetch(`${API}/api/carousel/jobs/${encodeURIComponent(jobId)}?_=${Date.now()}`,{cache:'no-store'});const data=await r.json().catch(()=>({}));if(run!==downloadRun)return;if(!r.ok)throw new Error(data.detail||text('Proses carousel gagal.','Carousel download failed.'));
      setProgress(data.progress||0,data.stage||text('Menyiapkan carousel','Preparing carousel'));
      if(data.state==='ready'){setProgress(100,text('Selesai','Done'));frame.src=`${API}/api/carousel/jobs/${encodeURIComponent(jobId)}/file?_=${Date.now()}`;if(button){button.disabled=false;button.textContent=button===carouselAll?'Download All (.zip)':text('Download item','Download item')}setTimeout(()=>{if(run===downloadRun)hideProgress()},2400);return}
