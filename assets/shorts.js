(()=>{
  ...
  if(!document.querySelector('#rvl-shorts-mobile-css')){
    const style=document.createElement('style');style.id='rvl-shorts-mobile-css';style.textContent=`
      body[data-platform="youtube-shorts"] #media-card:not(.show){display:none!important}
      body[data-platform="youtube-shorts"] #media-thumb:not(.ready){visibility:hidden!important}
      .media-size{font-size:11px;color:#b7c9ff;margin-top:5px;line-height:1.4;font-weight:600}
      @media(max-width:700px){
        ...
        body[data-platform="youtube-shorts"] .media-meta,body[data-platform="youtube-shorts"] .media-size{font-size:10px!important;line-height:1.4!important;margin-top:5px!important}
        ...
        body[data-platform="youtube-shorts"] .quality-select,body[data-platform="youtube-shorts"] #download-btn{width:100%!important;min-width:0!important;min-height:46px!important;margin:0!important}
        ...
        body[data-platform="youtube-shorts"] .rvl-pixel-buddy,body[data-platform="youtube-shorts"] .px-star,body[data-platform="youtube-shorts"] .px-cloud{display:none!important}
        body[data-platform="youtube-shorts"] .back-row{margin-top:15px!important}
      }
      @media(max-width:380px){body[data-platform="youtube-shorts"] .downloader-head h1{font-size:28px!important}body[data-platform="youtube-shorts"] .downloader-head p{font-size:12px!important}}
    `;document.head.appendChild(style);
  }

  let current=null,inspectRun=0,downloadRun=0,inspectController=null,sizeRun=0;
  let sizeEl=qs('#media-size');if(!sizeEl&&metaEl){sizeEl=document.createElement('div');sizeEl.id='media-size';sizeEl.className='media-size';metaEl.insertAdjacentElement('afterend',sizeEl)}
  ...
  function roughSize(){if(!current)return 0;const d=Number(current.data.duration||0);if(!d)return 0;const h=quality.value==='best'?Number(current.data.max_height||1080):Number(quality.value||1080);const mbps=h>=2160?25:h>=1440?12:h>=1080?6:h>=720?3:1.5;return d*((mbps*1e6)+192000)/8}
  async function refreshSize(){const run=++sizeRun;if(!current){setSize();return}const choice=(current.data.choices||[]).find(c=>String(c.id)===String(quality.value));if(choice?.estimated_bytes){setSize(choice.estimated_bytes,true);return}setSize(0,true,true);try{const r=await fetch(`${RVL_API}/api/file-size`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url:current.apiUrl,quality:quality.value}),cache:'no-store'});const data=await r.json().catch(()=>({}));if(run!==sizeRun||!current)return;if(r.ok&&data.bytes){setSize(data.bytes,data.estimated!==false);return}}catch{}if(run===sizeRun)setSize(roughSize(),true)}
  function canonicalShort(raw){try{const u=new URL(raw);const host=u.hostname.toLowerCase().replace(/^www\./,'').replace(/^m\./,'');if(host!=='youtube.com')return null;const m=u.pathname.match(/^\/shorts\/([A-Za-z0-9_-]{11})(?:\/|$)/);if(!m)return null;return `https://www.youtube.com/watch?v=${m[1]}`}catch{return null}}
  ...
  function reset(){current=null;++sizeRun;card.classList.remove('show');downloadBtn.disabled=true;quality.disabled=true;quality.innerHTML='';if(thumb){thumb.classList.remove('ready');thumb.removeAttribute('src')}if(titleEl)titleEl.textContent='';if(metaEl)metaEl.textContent='';if(sizeEl)sizeEl.textContent='';hideProgress()}
  if(thumb){thumb.addEventListener('load',()=>thumb.classList.add('ready'));thumb.addEventListener('error',()=>{thumb.classList.remove('ready');thumb.removeAttribute('src')})}
  function applyStaticCopy(){const en=lang()==='en',sub=qs('#page-sub');if(sub)sub.textContent=en?'Paste a YouTube Shorts link, check it, then choose the available video quality.':'Tempel link YouTube Shorts, cek videonya, lalu pilih kualitas yang tersedia.';input.placeholder=en?'Paste YouTube Shorts link':'Tempel link YouTube Shorts';inspectBtn.textContent=en?'Check':'Cek';if(!downloadBtn.disabled)downloadBtn.textContent='Download';if(current)refreshSize()}
  async function inspect(){const raw=input.value.trim(),apiUrl=canonicalShort(raw),run=++inspectRun;++downloadRun;if(inspectController)inspectController.abort();inspectController=new AbortController();reset();setStatus('');if(!apiUrl){setStatus(text('Tempel link youtube.com/shorts/... yang valid.','Paste a valid youtube.com/shorts/... link.'),'error');return}inspectBtn.disabled=true;input.disabled=true;setProgress(22,text('Mengecek Shorts','Checking Short'),{indeterminate:true});let timedOut=false;const timeout=setTimeout(()=>{timedOut=true;inspectController?.abort()},30000);try{const r=await fetch(`${RVL_API}/api/info`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url:apiUrl}),signal:inspectController.signal,cache:'no-store'});const data=await r.json().catch(()=>({}));if(run!==inspectRun)return;if(!r.ok)throw new Error(data.detail||text('Shorts nggak bisa dibaca.','Could not read this Short.'));current={raw,apiUrl,data};if(thumb&&data.thumbnail){thumb.classList.remove('ready');thumb.src=data.thumbnail;thumb.alt=data.title||'Short thumbnail'}if(titleEl)titleEl.textContent=data.title||'Untitled';const bits=[];if(data.uploader)bits.push(data.uploader);if(data.duration)bits.push(fmtDuration(data.duration));bits.push('Shorts');if(data.max_height)bits.push(`max ${data.max_height}p`);if(metaEl)metaEl.textContent=bits.join(' · ');const choices=(data.choices||[]).filter(c=>c.id!=='audio');if(!choices.length)throw new Error(text('Kualitas video Shorts tidak ditemukan.','No Shorts video quality found.'));choices.forEach(c=>{const o=document.createElement('option');o.value=c.id;o.textContent=c.label;quality.appendChild(o)});quality.disabled=false;downloadBtn.disabled=false;downloadBtn.textContent='Download';card.classList.add('show');hideProgress();refreshSize()}catch(e){if(run!==inspectRun)return;if(e.name==='AbortError'&&!timedOut)return;console.error(e);reset();setStatus(timedOut?text('Server YouTube terlalu lama merespons. Coba lagi sebentar.','YouTube server took too long. Please try again shortly.'):(e.message||text('Gagal mengecek Shorts.','Failed to check Short.')),'error')}finally{clearTimeout(timeout);if(run===inspectRun){inspectBtn.disabled=false;input.disabled=false;inspectController=null}}}
  async function pollJob(jobId,run){while(run===downloadRun){await sleep(650);const r=await fetch(`${RVL_API}/api/jobs/${encodeURIComponent(jobId)}?_=${Date.now()}`,{cache:'no-store'});const data=await r.json().catch(()=>({}));if(run!==downloadRun)return;if(!r.ok)throw new Error(data.detail||text('Proses Shorts gagal.','Shorts download failed.'));setProgress(data.progress||0,data.stage||text('Menyiapkan','Preparing'));if(data.state==='ready'){setProgress(100,text('Selesai','Done'));frame.src=`${RVL_API}/api/jobs/${encodeURIComponent(jobId)}/file-reyval?_=${Date.now()}`;downloadBtn.disabled=false;quality.disabled=false;downloadBtn.textContent='Download';setTimeout(()=>{if(run===downloadRun)hideProgress()},2600);return}if(data.state==='error')throw new Error(data.error||text('Download Shorts gagal.','Shorts download failed.'))}}
  async function download(){if(!current)return;const run=++downloadRun,selected=quality.value;setStatus('');downloadBtn.disabled=true;quality.disabled=true;downloadBtn.textContent=text('Proses…','Processing…');setProgress(3,text('Mulai proses','Starting'));try{const r=await fetch(`${RVL_API}/api/jobs`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url:current.apiUrl,quality:selected}),cache:'no-store'});const data=await r.json().catch(()=>({}));if(run!==downloadRun)return;if(!r.ok||!data.job_id)throw new Error(data.detail||text('Gagal memulai Shorts.','Could not start Shorts download.'));setProgress(data.progress||3,text('Antrean','Queued'));await pollJob(data.job_id,run)}catch(e){if(run!==downloadRun)return;console.error(e);hideProgress();setStatus(e.message||text('Download Shorts gagal.','Shorts download failed.'),'error');downloadBtn.disabled=false;quality.disabled=false;downloadBtn.textContent='Download'}}
  inspectBtn.addEventListener('click',inspect);downloadBtn.addEventListener('click',download);quality.addEventListener('change',refreshSize);input.addEventListener('keydown',e=>{if(e.key==='Enter')inspect()});input.addEventListener('input',()=>{if(current&&input.value.trim()!==current.raw){++inspectRun;++downloadRun;if(inspectController)inspectController.abort();reset();setStatus('')}});window.addEventListener('reyval:lang',applyStaticCopy);applyStaticCopy();
})();