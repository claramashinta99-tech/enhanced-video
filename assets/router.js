(()=>{
  ...
  function loadRouteScript(path){document.querySelectorAll('script[data-rvl-route-script]').forEach(s=>s.remove());document.querySelector('#rvl-download-frame')?.remove();let src=null,type=null;if(path==='/clarity/'){src=`/assets/clarity.js?route=${Date.now()}`;type='module'}else if(path==='/tiktok-downloader/'||path==='/youtube-downloader/'){src=`/assets/downloader.js?route=${Date.now()}`}if(!src)return;const s=document.createElement('script');s.dataset.rvlRouteScript='1';s.src=src;if(type)s.type=type;document.body.appendChild(s)}
