(()=>{
  ...
  function applyStaticCopy(){
    ...
    inspectBtn.textContent=en?'Check':'Cek';downloadBtn.textContent='Download';if(!current)setStatus('');
  }

  function resetMediaCard(){current=null;card.classList.remove('show');downloadBtn.disabled=true;quality.disabled=true;thumb?.removeAttribute('src');if(titleEl)titleEl.textContent='';if(metaEl)metaEl.textContent='';quality.innerHTML=''}

  async function inspect(){
    const url=input.value.trim();const run=++inspectRun;
    ...
    try{
      ...
      quality.disabled=false;downloadBtn.disabled=false;card.classList.add('show');setStatus('');
  ...
  function download(){
    if(!current)return;const selected=quality.value;
    const directURL=`${RVL_API}/api/download?url=${encodeURIComponent(current.url)}&quality=${encodeURIComponent(selected)}&_=${Date.now()}`;
    setStatus(text('Menyiapkan file dari hasil cek tadi…','Preparing from the checked result…'));frame.src=directURL;
    setTimeout(()=>setStatus(text('Kalau file sudah mulai masuk, proses server selesai.','Once the file starts downloading, server prep is done.'),'ok'),1800);
  }
