(()=>{
  ...
  const RVL_API=platform==='youtube'?'https://enhanced-video-production.up.railway.app':'https://rvl-api.onrender.com';
  let current=null,inspectRun=0,inspectController=null,downloadRun=0,sizeRun=0;

  if(!document.querySelector('#rvl-downloader-style')){
    const css=document.createElement('style');css.id='rvl-downloader-style';css.textContent=`
  ...
  document.querySelector('#rvl-download-frame')?.remove();const frame=document.createElement('iframe');frame.name='rvl-download-frame';frame.id='rvl-download-frame';frame.style.display='none';document.body.appendChild(frame);
  ...
  function lang(){return localStorage.getItem('reyval-lang')||'id'}function text(id,en){return lang()==='en'?en:id}
  ...
  function applyStaticCopy(){const en=lang();const sub=qs('#page-sub');if(sub)sub.textContent=en==='en'?(platform==='youtube'?'Paste a regular YouTube video link. Use YouTube Shorts for Shorts links.':'Paste a TikTok link, check it, then choose the video quality.'):(platform==='youtube'?'Tempel link video YouTube biasa. Link Shorts pakai tool YouTube Shorts.':'Tempel link TikTok, cek videonya, lalu pilih kualitas video.');input.placeholder=en==='en'?`Paste ${platform==='youtube'?'YouTube':'TikTok'} link`:`Tempel link ${platform==='youtube'?'YouTube':'TikTok'}`;inspectBtn.textContent=en==='en'?'Check':'Cek';if(!downloadBtn.disabled)downloadBtn.textContent='Download';if(!current)setStatus('');else refreshSize()}
  function resetMediaCard(){current=null;++sizeRun;card.classList.remove('show','no-thumb');downloadBtn.disabled=true;quality.disabled=true;if(thumb){thumb.onerror=null;thumb.onload=null;thumb.removeAttribute('src');thumb.style.display=''}if(titleEl)titleEl.textContent='';if(metaEl)metaEl.textContent='';if(sizeEl)sizeEl.textContent='';quality.innerHTML='';hideProgress()}
  function setThumbnail(url,title){
    if(!thumb)return;
    if(!url){card.classList.add('no-thumb');thumb.removeAttribute('src');return}
    card.classList.remove('no-thumb');thumb.style.display='';thumb.alt=title||'Media thumbnail';
    thumb.onerror=()=>{card.classList.add('no-thumb');thumb.removeAttribute('src')};
    thumb.onload=()=>card.classList.remove('no-thumb');
    thumb.src=url;
  }
  ...
  async function inspect(){
    ...
    if(platform==='youtube'&&isShortUrl(url)){setStatus(text('Ini link Shorts. Pakai tool YouTube Shorts.','This is a Shorts link. Use the YouTube Shorts tool.'),'error');return}
    ...
    try{
      const r=await fetch(`${RVL_API}/api/info`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url}),signal:inspectController.signal,cache:'no-store'});const data=await r.json().catch(()=>({}));if(run!==inspectRun)return;if(!r.ok)throw new Error(data.detail||text('Link nggak bisa dibaca.','Could not read this link.'));
      current={url,data};setThumbnail(data.thumbnail,data.title);if(titleEl)titleEl.textContent=data.title||'Untitled';const bits=[];if(data.uploader)bits.push(data.uploader);if(data.duration)bits.push(fmtDuration(data.duration));if(platform==='youtube'&&data.max_height)bits.push(`max ${data.max_height}p`);if(metaEl)metaEl.textContent=bits.join(' · ');
      const choices=(data.choices||[]).filter(c=>c.id!=='audio');if(!choices.length)throw new Error(text('Kualitas video tidak ditemukan.','No video quality found.'));choices.forEach(c=>{const o=document.createElement('option');o.value=c.id;o.textContent=c.label;quality.appendChild(o)});quality.disabled=false;downloadBtn.disabled=false;downloadBtn.textContent='Download';card.classList.add('show');hideProgress();setStatus('');refreshSize();
  ...
  async function pollJob(jobId,run){while(run===downloadRun){await sleep(650);const r=await fetch(`${RVL_API}/api/jobs/${encodeURIComponent(jobId)}?_=${Date.now()}`,{cache:'no-store'});const data=await r.json().catch(()=>({}));if(run!==downloadRun)return;if(!r.ok)throw new Error(data.detail||text('Proses download gagal.','Download process failed.'));setProgress(data.progress||0,data.stage||text('Menyiapkan','Preparing'));if(data.state==='ready'){setProgress(100,text('Selesai','Done'));frame.src=`${RVL_API}/api/jobs/${encodeURIComponent(jobId)}/file-reyval?_=${Date.now()}`;downloadBtn.disabled=false;quality.disabled=false;downloadBtn.textContent='Download';setTimeout(()=>{if(run===downloadRun)hideProgress()},2600);return}if(data.state==='error')throw new Error(data.error||text('Download gagal.','Download failed.'))}}
  ...
  inspectBtn.addEventListener('click',inspect);downloadBtn.addEventListener('click',download);quality.addEventListener('change',refreshSize);input.addEventListener('keydown',e=>{if(e.key==='Enter')inspect()});input.addEventListener('input',()=>{if(current&&input.value.trim()!==current.url){++inspectRun;++downloadRun;if(inspectController)inspectController.abort();resetMediaCard();setStatus('')}});window.addEventListener('reyval:lang',applyStaticCopy);applyStaticCopy();
})();
