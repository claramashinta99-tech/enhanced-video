...
def _audio_source_from_info(info):
    ...
    for fmt in info.get('formats') or []:
        ...
        try:
            ...
            abr = 0
        ext = fmt.get('ext') or ''
        ext_rank = 3 if ext == 'm4a' else 2 if ext in {'mp4', 'webm'} else 1
        audio_only = 1 if vcodec in {None, 'none'} else 0
...
def _mp3_attempts():
    ...
    preferred = []
    for name in ('mweb-cookie', 'default-cookie', 'safari-cookie'):
        preferred.extend([s for s in attempts if s.get('name') == name])
    if preferred:
        return preferred
    return attempts[:2]
    ...
    for strategy in _mp3_attempts():
        try:
            ...
            return {
                ...
                'duration': (info or {}).get('duration') or source.get('duration'),
...
def _ensure_source_prepare(url):
    ...
    if created:
        threading.Thread(target=_source_worker, args=(url, state), daemon=True).start()
    return state


def _wait_source(url):
    state = _ensure_source_prepare(url)
    if state.get('state') == 'ready' and state.get('source'):
        return state['source']
...
def _safe_title(value):
    value = re.sub(r'[\\/:*?"<>|]+', ' ', str(value or 'YouTube audio'))
...
def _transcode_direct(source_data, workdir, job_id=None):
    ...
    if job_id:
        legacy.job_update(job_id, state='working', progress=16, stage='Convert MP3')
    started = time.monotonic()
            ...
            for raw in proc.stdout:
                ...
                if line.startswith('out_time_ms=') and duration > 0 and job_id:
                    try:
                        ...
                        legacy.job_update(job_id, state='working', progress=18 + int(ratio * 77), stage='Convert MP3')
                    except (TypeError, ValueError):
                        pass
    ...
    if job_id:
        legacy.job_update(job_id, state='working', progress=97, stage='Finalisasi')
    elapsed = int((time.monotonic() - started) * 1000)
    print(f'mp3 direct transcode done ms={elapsed} bytes={output.stat().st_size}', flush=True)
    return output
...
def fast_mp3_sync(url, workdir, job_id=None):
    ...
    try:
        source_data = _wait_source(url)
        return _transcode_direct(source_data, workdir, job_id)
    except Exception as exc:
        print(f'mp3 direct pipeline fallback type={type(exc).__name__}', flush=True)
...
async def create_mp3_job(body: legacy.DownloadBody):
    ...
    return {'job_id': job_id, 'state': 'queued', 'progress': 2, 'fast_path': 'direct-transcode'}
